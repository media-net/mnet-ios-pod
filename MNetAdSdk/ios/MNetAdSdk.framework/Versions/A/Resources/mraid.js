...
for (var i = 0; i < args.length; i += 2) {
 ...
 mraid.playVideo = function(url) {
    ...
    if (!url) {
        broadcastEvent(EVENTS.ERROR, 'playVideo must be called with a valid url', 'playVideo');
    } else {
 ...
 mraid.storePicture = function(url) {
    ...
    if (!url) {
        broadcastEvent(EVENTS.ERROR, 'storePicture must be called with a valid url', 'storePicture');
    } else {
        bridge.executeNativeCall(['storePicture', 'url', url]);
    }
 };
